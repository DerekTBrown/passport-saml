...
SAML.prototype.generateAuthorizeRequest = function (req, isPassive, callback) {
  ...
  .then(function(){
    ...
    } else if (self.options.NameIDAllowCreate){
     request['samlp:AuthnRequest']['samlp:NameIDPolicy'] = {
        '@AllowCreate': '1'
      };
    }
    ...
    callback(null, xmlbuilder.create(request, {}, {}, { headless: true }).end());
  })
  .fail(function(err){
    callback(err);
  })
...
SAML.prototype.generateServiceProviderMetadata = function( decryptionCert ) {
  ...
  return xmlbuilder.create(metadata, {}, {}, { headless: true}).end({ pretty: true, indent: '  ', newline: '\n' });
};
